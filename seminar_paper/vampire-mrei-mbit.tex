...
Vereinfachend kann das if-then-else-Konstrukt $ \$ite\_f $ für Formeln und $\$ite\_t$ für Terme benutzt werden:
...
$tff(c2,axiom,\\
\textbf{\$let\_ff}(p(X),q(X)|r(X),p(c)) \& \sin q(c)\& \sin r(c)).$\\

$tff(c3,axiom,\\
\textbf{\$let\_tf}(f(X),g(X),p(f(X)))\& \sin p(g(X))).$\\
